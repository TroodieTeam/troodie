-- ============================================================================
-- Fix get_creators() VARCHAR to TEXT type casting
-- ============================================================================
-- Fixes PostgreSQL error: "Returned type character varying does not match 
-- expected type text in column 3"
-- 
-- Issue: COALESCE with VARCHAR columns preserves VARCHAR type even with ::TEXT cast.
-- Solution: Cast each COALESCE argument individually to TEXT.
-- Date: 2025-02-05
-- ============================================================================
...
BEGIN
  ...
  SELECT
    cp.id,
    cp.user_id,
    -- Fix: Cast each COALESCE argument individually to TEXT to avoid VARCHAR type inference
    COALESCE(
      cp.display_name::TEXT,
      u.name::TEXT,
      u.username::TEXT
    ) as display_name,
    COALESCE(cp.bio, '')::TEXT as bio,
    COALESCE(cp.location, '')::TEXT as location,
    COALESCE(u.avatar_url, '')::TEXT as avatar_url,
    cp.total_followers,
    cp.troodie_engagement_rate,
    cp.open_to_collabs,
    COALESCE(cp.availability_status, 'available')::TEXT as availability_status,
    cp.specialties,
    (
  ...
  WHERE cp.open_to_collabs = true
    ...
    AND (p_city IS NULL OR LOWER(COALESCE(cp.location, '')::TEXT) LIKE LOWER('%' || p_city || '%'))
...
'Returns filtered list of creators with availability_status. CRITICAL: Only includes users with account_type = ''creator''. Business users with creator_profiles are excluded. Test users are hidden from production users but visible to other test users. Fixed VARCHAR to TEXT casting for all text columns.';
