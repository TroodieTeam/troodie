...
interface PostCommentsProps {
  ...
  bottomOffset?: number;
  comments?: CommentWithUser[]; // Pass comments from parent for sync
  onCommentsChange?: (comments: CommentWithUser[]) => void; // Callback when comments change
}
          ...
          if (userError || !userData) {
            ...
            if (authUser && authUser.user) {
              userData = {
                ...
                persona:
                  authUser.user.user_metadata?.persona || "Food Explorer",
        ...
        <View style={styles.twitterInputContainer}>
          ...
          {postAuthorName && (
            <Text style={styles.replyingToText}>
              Replying to{" "}
              <Text style={styles.replyingToUsername}>@{postAuthorName}</Text>
            </Text>
          )}
          <View style={styles.inputWrapper}>
            <TextInput
              ...
              returnKeyType="send"
              onSubmitEditing={
                newComment.trim() ? handleSubmitComment : undefined
              }
              blurOnSubmit={false}
            />
            <TouchableOpacity
      ...
      <View style={styles.commentsOnlyContainer}>
        {loading ? (
          ...
          comments.slice(0, 10).map(
            (
              item // Show more comments when separate
        ...
        <View style={styles.commentsList}>
          {comments.length === 0 ? (
            ...
            comments.slice(0, 5).map(
              (
                item // Limit to 5 comments to avoid height issues
              ) => <View key={item.id}>{renderComment({ item })}</View>
            )
          )}
          {comments.length > 5 && (
