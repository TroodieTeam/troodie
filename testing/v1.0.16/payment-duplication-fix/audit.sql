...
SELECT ca.id AS app_id, c.title, u.full_name, u.email,
  ...
  cp.creator_payout_cents AS expected_cents
FROM campaign_applications ca
JOIN campaign_deliverables cd ON cd.campaign_application_id = ca.id
JOIN campaigns c ON c.id = ca.campaign_id
JOIN users u ON u.id = ca.creator_id
LEFT JOIN campaign_payments cp ON cp.campaign_id = ca.campaign_id AND cp.status = 'succeeded'
GROUP BY ca.id, c.title, u.full_name, u.email, cp.creator_payout_cents
