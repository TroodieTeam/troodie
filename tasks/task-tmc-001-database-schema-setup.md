# Troodie-Managed Campaigns: Database Schema Setup
...
## Overview
Extend the existing database schema to support Troodie-managed campaigns, including platform-managed restaurants, campaign source tracking, subsidy information, and partnership metadata. This enables Troodie to create and manage campaigns as a "virtual restaurant" to solve the creator cold-start problem.

## Business Value
- Enables Troodie to create campaigns when restaurant supply is low
- Provides foundation for transparent platform-managed campaigns
- Supports partnership model where Troodie subsidizes restaurant campaigns
- Allows tracking of campaign budget sources and ROI
- Critical infrastructure for marketplace MVP launch

## Acceptance Criteria (Gherkin)
```gherkin
Feature: Troodie-Managed Campaign Database Schema
  As a platform administrator
  I want database support for Troodie-managed campaigns
  So that we can create campaigns without restaurant partners

  Scenario: Create platform-managed restaurant
    Given I have admin access to the database
    When I create a new restaurant with is_platform_managed=true
    Then the restaurant is marked as Troodie-owned
    And it can be used to create Troodie campaigns
    And it has the managed_by field set to 'troodie'

  Scenario: Track campaign source
    Given a campaign exists in the database
    When I query the campaign_source field
    Then I can determine if it's 'restaurant', 'troodie_direct', 'troodie_partnership', or 'community_challenge'
    And I can filter campaigns by source type

  Scenario: Record subsidy information
    Given a partnership campaign exists
    When the campaign has is_subsidized=true
    Then the subsidy_amount_cents is tracked
    And the budget_source is recorded
    And internal notes and metrics are stored

  Scenario: Query platform-managed campaigns
    Given multiple campaigns exist with different sources
    When I query for Troodie-managed campaigns
    Then I get all campaigns where campaign_source != 'restaurant'
    And I can see which are direct vs partnerships vs challenges
    And subsidy information is included for partnerships
```

## Technical Implementation

### Database Migration File
Create: `supabase/migrations/YYYYMMDD_troodie_managed_campaigns_schema.sql`
...
CREATE TABLE IF NOT EXISTS platform_managed_campaigns (
  ...
  -- Internal management
  internal_notes TEXT,
  campaign_manager_user_id UUID REFERENCES auth.users(id),
...
-- Insert Troodie official "restaurant"
INSERT INTO restaurants (
  ...
  'troodie-official-restaurant-id',
  'Troodie Community',
  'Official Troodie campaigns, challenges, and creator opportunities',
  'Charlotte',
  'NC',
  TRUE,
  'troodie'
...
BEGIN
  ...
  RAISE NOTICE '1. Create Troodie system user account';
  RAISE NOTICE '2. Build admin campaign creation UI';
  RAISE NOTICE '3. Update creator campaign browsing to show Troodie campaigns';
  RAISE NOTICE '';
END $$;
```
...
- [ ] All new columns added with proper constraints
- [ ] platform_managed_campaigns table created
- [ ] RLS policies implemented and tested
- [ ] Triggers for spend tracking functional
- [ ] Triggers for metrics tracking functional
- [ ] Summary view created and queryable
- [ ] Troodie official restaurant record inserted
- [ ] TypeScript types updated
- [ ] Migration tested with rollback
- [ ] Database indexes created for performance
- [ ] Comments added for all new columns/tables
- [ ] Migration pushed to remote Supabase instance

## Notes
- **Database-only task** - no UI changes
- Sets foundation for all Troodie-managed campaign features
- Maintains backward compatibility with existing campaigns
- Partnership tracking supports white-label model
- Budget tracking enables ROI analysis
- Triggers ensure metrics stay in sync automatically
- Reference: TROODIE_MANAGED_CAMPAIGNS_STRATEGY.md sections "Implementation Models" and "Technical Implementation"
- Related tasks: TMC-002 (System Account), TMC-003 (Admin UI), TMC-004 (Creator UI)
